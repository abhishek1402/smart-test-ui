...
ipcMain.handle('recordTest', async (event:any, {testCase,name, preTestId}:{testCase:string,name:string, preTestId: string}) => {
  // mainWindow.webContents.send('test123',"sdfsfd");
 
  const testCases = await TestServies.recordTestCase({mainWindow,testCase,name, preTestId});
  return "abhishek"
...
ipcMain.handle('recordTestOnLocal', async (event:any, arg:any) => {
  ...
  const testCases = await TestServies.recordTestCaseOnLocal({mainWindow, preTestId: arg.preTestId});
  return "abhishek"
